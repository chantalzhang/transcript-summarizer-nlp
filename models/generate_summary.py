...
# nltk.download('punkt')  # Uncomment if NLTK is not initialized
warnings.filterwarnings("ignore", category=ResourceWarning)  # Suppress ResourceWarnings

# Abstractive summarization model
summarizer = pipeline("summarization", model="google/pegasus-xsum", device=0)  # Use GPU if available

# Load environment variables
load_dotenv()

# Weaviate and API credentials
wcd_url = os.getenv("WCD_URL")
wcd_api_key = os.getenv("WCD_API_KEY")
huggingface_key = os.getenv("HF_KEY")
...
# Connect to Weaviate
client = wc.Client(wc.AuthApiKey(api_key=wcd_api_key, cluster_url=wcd_url), headers=headers)


def normalize_sentence(sentence):
    """Normalize a sentence for comparison."""
    sentence = unicodedata.normalize("NFC", sentence).strip().lower()
    return re.sub(r"\s+", " ", sentence)


def retrieve_relevant_sentences(client, collection_name, query_text, num_results=5):
    """Retrieve relevant sentences from Weaviate."""
    collection = client.collection(collection_name)
    response = collection.query().with_near_text({"query": query_text, "distance": 0.8}).with_limit(num_results).do()
    
    seen_sentences = {normalize_sentence(query_text)}
    sentences = [
        normalize_sentence(obj["properties"]["sentence"])
        for obj in response.get("data", {}).get("Get", {}).get(collection_name, [])
        if normalize_sentence(obj["properties"]["sentence"]) not in seen_sentences
    ]
    return sentences
...
def hierarchical_summarization_pipeline(lecture_text, output_dir, filename_prefix, max_words_per_chunk=500):
    ...
    # Step 3: Hierarchical summarization (if summaries are too long)
    while sum(len(summary.split()) for summary in summaries) > 500:
        summaries = summarize_chunks(split_into_chunks(" ".join(summaries)))

    # Step 4: Save the final summary
    final_summary = "\n".join(summaries)
    save_to_file(final_summary, output_dir, f"{filename_prefix}_final_summary.txt")
    return final_summary
...
def save_to_file(content, output_dir, filename):
    ...
    with open(os.path.join(output_dir, filename), "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Summary saved to {os.path.join(output_dir, filename)}")


if __name__ == "__main__":
    # Path to lecture file and output settings
    lecture_file = "path/to/lecture.txt"  # Replace with your file path
    collection_name = "collection_name"  # Replace with your Weaviate collection name
    output_directory = "summaries"
    filename_prefix = "lecture"

    # Read lecture text
    with open(lecture_file, "r", encoding="utf-8") as file:
        lecture_text = file.read()

    # Run summarization pipeline
    final_summary = hierarchical_summarization_pipeline(lecture_text, output_directory, filename_prefix)
    print("Final Summary:")
    print(final_summary)
